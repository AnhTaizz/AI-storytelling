...
# OUTPUT FORMAT
You must return PLAIN YAML TEXT only. 
- Do not use a Markdown code block.
- Do not add prose before or after the YAML.
- Do not emit Markdown fencing.
- The response must begin directly with:
  critic_version: H7_EIC_V2
- The response must not contain the U+0060 GRAVE ACCENT character anywhere.
- The first non-whitespace content must be:
  critic_version: H7_EIC_V2
- Output the YAML document directly as the assistant response body.
...
# SELF-CHECK
Before returning the answer, mechanically verify:
1. first non-whitespace line is exactly:
   critic_version: H7_EIC_V2
2. response contains zero U+0060 GRAVE ACCENT characters;
3. no Markdown code block exists;
4. no prose exists before or after the YAML;
5. YAML structure follows the required schema.

Additionally, verify internally:
...
Do NOT print your self-check reasoning. Return ONLY the PLAIN YAML TEXT output.
