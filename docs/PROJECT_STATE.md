...
M1-30CH-M is PARTIALLY_SUPPORTED. Reranking the K Top-30 improved hit, recall and MRR, but not Full Evidence Success@10, which remained 4 of the 12 candidate-complete probes. Before another intervention, run a diagnostic on the M rankings (no new model) over the 8 candidate-complete probes that still lack full evidence at Top-10. It would ask where their missing required units rank after reranking, and whether those units' passages were cut by the 512-token pair limit. The goal is to separate "multi-evidence assembly across passages" from "evidence text truncated away" as the reason full success stays flat. Not implemented. Reranker results alone do not justify graph memory.
